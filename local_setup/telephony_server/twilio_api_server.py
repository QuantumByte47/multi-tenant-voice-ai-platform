...
def populate_ngrok_tunnels():
    ...
    telephony_url, voice_agent_framework_url = None, None
    ...
    if response.status_code == 200:
        ...
        for tunnel in data['tunnels']:
            ...
            elif tunnel['name'] == 'voice-agent-framework-app':
                voice_agent_framework_url = tunnel['public_url'].replace('https:', 'wss:')

        return telephony_url, voice_agent_framework_url
    else:
...
async def make_call(request: Request):
    try:
        ...
        telephony_host, voice_agent_framework_host = populate_ngrok_tunnels()

        print(f'telephony_host: {telephony_host}')
        print(f'voice_agent_framework_host: {voice_agent_framework_host}')

        try:
            call = twilio_client.calls.create(
                to=call_details.get('recipient_phone_number'),
                from_=twilio_phone_number,
                url=f"{telephony_host}/twilio_connect?voice_agent_framework_host={voice_agent_framework_host}&agent_id={agent_id}",
                method="POST",
                record=True
            )
...
async def twilio_connect(voice_agent_framework_host: str = Query(...), agent_id: str = Query(...)):
    try:
        response = VoiceResponse()

        connect = Connect()
        voice_agent_framework_websocket_url = f'{voice_agent_framework_host}/chat/v1/{agent_id}'
        connect.stream(url=voice_agent_framework_websocket_url)
        print(f"websocket connection done to {voice_agent_framework_websocket_url}")
